...
#'
#' @title
#'     Plotting MCMC results for the function \code{DynMultiNet}
#'
#' @description
...
#'            \item{\code{"x_iht_shared"}}{Global (shared) latent coordinates for node_i in dimension h.}
#'            \item{\code{"x_ihtk"}}{Layer-specific latent coordinates for node_i at layer_k in dimension h.}
...
plot.DynMultiNet_mcmc <- function( DynMultiNet_mcmc,
  ...
  directed <- DynMultiNet_mcmc$directed
  weighted <- DynMultiNet_mcmc$weighted
  V_net <- dim(DynMultiNet_mcmc$y_ijtk)[1]
  T_net <- dim(DynMultiNet_mcmc$y_ijtk)[3]
  K_net <- dim(DynMultiNet_mcmc$y_ijtk)[4]
  if(directed){
    H_dim <- dim(DynMultiNet_mcmc$tau_h_shared_mcmc[[1]])[1]
    R_dim <- dim(DynMultiNet_mcmc$tau_h_k_mcmc[[1]])[1]
  } else {
    H_dim <- dim(DynMultiNet_mcmc$tau_h_shared_mcmc)[1]
    R_dim <- dim(DynMultiNet_mcmc$tau_h_k_mcmc)[1]
  }
  n_iter_mcmc <- dim(DynMultiNet_mcmc$mu_tk_mcmc)[3]
  ...
  if( is.element(param,"mu_tk") ){
    ...
    if(is.null(layer_k)){ layer_k=DynMultiNet_mcmc$layer_all[1]; warning("Plotting layer_k=",layer_k," as layer_k was not specified") }
    if(!is.element(layer_k,DynMultiNet_mcmc$layer_all)) {stop("layer_k=",layer_k," is not a valid layer")}
    k <- match(layer_k,DynMultiNet_mcmc$layer_all)
    
    param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$mu_tk_mcmc[,k,iter_out_mcmc]) )
    
  } else if( is.element(param,"pi_ijtk") ){
    
    if(is.null(node_i)){ node_i=DynMultiNet_mcmc$node_all[2]; warning("Plotting node_i=",node_i," as node_i was not specified") }
    if(!is.element(node_i,DynMultiNet_mcmc$node_all)) {stop("node_i=",node_i," is not a valid node")}
    i <- match(node_i,DynMultiNet_mcmc$node_all)
    
    if(is.null(node_j)){ node_j=DynMultiNet_mcmc$node_all[1]; warning("Plotting node_j=",node_j," as node_j was not specified") }
    if(!is.element(node_j,DynMultiNet_mcmc$node_all)) {stop("node_i=",node_j," is not a valid node")}
    j <- match(node_j,DynMultiNet_mcmc$node_all)
    
    if(is.null(layer_k)){ layer_k=DynMultiNet_mcmc$layer_all[1]; warning("Plotting layer_k=",layer_k," as layer_k was not specified") }
    if(!is.element(layer_k,DynMultiNet_mcmc$layer_all)) {stop("node_i=",layer_k," is not a valid node")}
    k <- match(layer_k,DynMultiNet_mcmc$layer_all)
    
    param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$pi_ijtk_mcmc[i,j,,k,iter_out_mcmc]) )
    
  } else if( is.element(param,"x_iht_shared") ) {
    ...
    if(directed){
      ...
      param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$x_iht_shared_mcmc[[dir]][i,h,,iter_out_mcmc]) )
    } else {
      param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$x_iht_shared_mcmc[i,h,,iter_out_mcmc]) )
    }
    
  } else if( is.element(param,"x_ihtk") ) {
    ...
    if(directed){
      ...
      if(!is.element(lat_space,c("send","receive"))) {stop("lat_space=",lat_space,' must be "send" or "receive".')}
      dir <- match(lat_space,c("send","receive"))
      param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$x_ihtk_mcmc[[dir]][i,h,,k,iter_out_mcmc]) )
    } else {
      param_mcmc_chain <- coda::mcmc( t(DynMultiNet_mcmc$x_ihtk_mcmc[i,h,,k,iter_out_mcmc]) )
    }
  }
  ...
  summary_mcmc <- data.frame(cbind(time=DynMultiNet_mcmc$time_all,summary_mcmc[[1]],summary_mcmc[[2]]))
  ...
  if( is.element(param,"mu_tk") ){
    ...
    p <- p + geom_point( aes( y=DynMultiNet_mcmc$y_ijtk[i,j,,k],
                              x=DynMultiNet_mcmc$time_all) ) +
      labs(x="time",y="pi_ijtk",title="pi_ijtk",subtitle=paste(node_i,"->",node_j,", layer_k=",layer_k,sep=""))
  } else if( is.element(param,"x_iht_shared") ) {
    p <- p + labs(x="time",y="x_iht_shared",title="x_iht_shared",subtitle=paste("node_i=",node_i,", h=",h,sep=""))
  } else if( is.element(param,"x_ihtk") ) {
    p <- p + labs(x="time",y="x_ihtk",title="x_ihtk",subtitle=paste("node_i=",node_i,", h=",h,", layer_k=",layer_k,sep=""))
  }
