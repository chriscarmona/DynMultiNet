...
// [[Rcpp::export]]
Rcpp::List sample_x_iht_mat_DynMultiNet_bin_dir_cpp( arma::mat x_iht_send_mat,
                                                     arma::mat x_iht_receive_mat,
                                                     const arma::mat x_t_sigma_prior_inv,
  ...
  unsigned int V_net = y_ijt.n_rows;
  unsigned int T_net = y_ijt.n_slices;
  unsigned int H_dim = x_iht_send_mat.n_cols / T_net;
  unsigned int K_net = y_ijtk.n_rows;
  ...
  // initializing x_i_cov_prior, it will be changed for each direction
  arma::mat tau_h_diag(tau_h_send.n_rows,tau_h_send.n_rows); tau_h_diag.eye();
  arma::mat x_i_cov_prior( H_dim*(H_dim*T_net), H_dim*(H_dim*T_net) );
  ...
  aux_uvec_2 = T_net * arma::regspace<arma::uvec>( 0, H_dim-1 );
  aux_uvec_3 = arma::regspace<arma::uvec>( 0, V_net-1 );
  // Model matrix for SENDER, made out of receiver coordinates
  for( t=0; t<T_net; t++ ) {
    X_all.submat( aux_uvec_1+t , aux_uvec_2+t ) = x_iht_receive_mat.submat( aux_uvec_3, aux_uvec_2+t );
  }
  arma::sp_mat X_all_send_sp = arma::sp_mat(X_all);
  // Model matrix for RECEIVER, made out of sender coordinates
  X_all = arma::zeros<arma::mat>(V_net*T_net,T_net*H_dim);
  for( t=0; t<T_net; t++ ) {
    X_all.submat( aux_uvec_1+t , aux_uvec_2+t ) = x_iht_send_mat.submat( aux_uvec_3, aux_uvec_2+t );
  }
  arma::sp_mat X_all_receive_sp = arma::sp_mat(X_all);
  
  // Model matrix auxiliar for the sampling loop for each node
  arma::sp_mat X_sp = X_all_send_sp;
  
  for( dir_i=0; dir_i<2; dir_i++ ) {
    tau_h_diag.eye();
    if(dir_i==0){
    ...
    for( i=0; i<V_net; i++ ) {
      ...
      if( dir_i==0 ) {
        C = S - X_sp * trans(x_iht_send_mat.row(i));
      } else if( dir_i==1 ) {
        C = S - X_sp * trans(x_iht_receive_mat.row(i));
      }
      ...
      if( dir_i==0 ) {
        x_iht_send_mat.row(i) = trans(arma::mvnrnd( x_i_cov*aux_vec_mean , x_i_cov ));
      } else if( dir_i==1 ) {
        x_iht_receive_mat.row(i) = trans(arma::mvnrnd( x_i_cov*aux_vec_mean , x_i_cov ));
      }
    }
  }
  
  return Rcpp::List::create( Rcpp::Named("send") = x_iht_send_mat,
                             Rcpp::Named("receive") = x_iht_receive_mat );
}
