...
transformed parameters {
  ...
  for(t in 1:T_all) {
    for(k in 1:K_net) {
      s_ij_tk[t,k] = mu_tk[k][t] + x_ih_t_shared[1,t]*x_ih_t_shared[2,t]' + x_ih_tk[1,t,k]*x_ih_tk[2,t,k]';
      r_ij_tk[t,k] = lambda_tk[k][t] + u_ih_t_shared[1,t]*u_ih_t_shared[2,t]' + u_ih_tk[1,t,k]*u_ih_tk[2,t,k]';
    }
  }
...
model {
  ...
  // baseline processes //
  for (k in 1:K_net) {
    mu_tk[k] ~ multi_normal_cholesky( mu_tk_mean[,k] , mu_t_cov_prior_sqrt );
    lambda_tk[k] ~ multi_normal_cholesky( lambda_tk_mean[,k] , lambda_t_cov_prior_sqrt );
  }
