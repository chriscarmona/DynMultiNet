...
parameters {
  ...
  matrix[V_net,T_net] z_x_it_h_shared[2,H_dim];
  matrix[V_net,T_net] z_x_it_hk[2,R_dim,K_net];
...
transformed parameters {
  ...
  matrix[T_net,V_net] x_ti_h_shared[2,H_dim];
  matrix[T_net,V_net] x_ti_hk[2,R_dim,K_net];
  
  matrix[V_net,H_dim] x_ih_t_shared[2,T_net];
  matrix[V_net,R_dim] x_ih_tk[2,T_net,K_net];
  
  vector[H_dim] tau_h_shared[2];
  matrix[H_dim,K_net] tau_hk[2];
  cholesky_factor_cov[T_net] x_t_shared_cov_chol[2,H_dim];
  cholesky_factor_cov[T_net] x_t_k_cov_chol[2,H_dim,K_net];
