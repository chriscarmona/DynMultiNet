...
// [[Rcpp::export]]
Rcpp::List sample_coord_ith_link_nGP_cpp( arma::cube ab_ith,
...
// [[Rcpp::export]]
Rcpp::List sample_coord_ith_shared_link_nGP_cpp( arma::cube ab_ith,
                                                 arma::field<arma::cube> alpha_ab_ith,
  ...
  for( i=0; i<V_net; i++ ) {
    ...
    for( h=0; h<H_dim; h++ ) {
      ...
      aux_mat_1 = ab_ith.slice(h); aux_mat_1.shed_row(i);
      X_t.col(0) = repmat(aux_mat_1,3,1);
      // Constant term for theta in the linear predictor
      C_t = linpred_t; C_t.zeros();
      for( t=0; t<T_net; t++ ) {
        // Rcpp::Rcout << "t=" << t << std::endl;
